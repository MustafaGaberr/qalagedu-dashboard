...
export async function apiRequest<T>(path: string, options: Options = {}) {
  ...
  try { response = await fetch(apiUrl(path), { ...options, method, headers, credentials: "include", body: options.body === undefined ? undefined : multipart ? options.body as FormData : JSON.stringify(options.body) }); }
