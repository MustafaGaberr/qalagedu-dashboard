...
type TeacherInput = StaffAccountInput & { subject?: string; bio?: string; photoUrl?: string };
...
export function StaffProvider({ children }: { children: React.ReactNode }) {
  ...
  const load = useCallback(async () => {
    ...
    setTeachers(teacherRaw.map((item) => ({ id: item.id, name: item.name, loginIdentifier: item.loginIdentifier, subject: item.teacherProfile?.subject ?? "—", bio: item.teacherProfile?.bio ?? undefined, photoUrl: item.teacherProfile?.photoUrl ?? undefined, grades: [...new Set([...(item.coursesAsTeacher ?? []).map((course: any) => course.gradeId), ...(item.centerGroupsAsTeacher ?? []).map((group: any) => group.gradeId)])] as string[], courseIds: (item.coursesAsTeacher ?? []).map((course: any) => course.id), groupIds: (item.centerGroupsAsTeacher ?? []).map((group: any) => group.id), status: item.status })));
